...
// Meta estática (no traducible): imágenes, hrefs, precisión, códigos.
// El `name` y `tagline` vienen del i18n vía `useI18n()` dentro del
// componente, así se traducen automáticamente y respetan `\n` de
// line-break controlado.
    ...
    <section className="container-x pt-16 pb-24">
      ...
      <div
        className={`mt-14 grid gap-6 ${
          ...
          // orb-halo-stacked: los productos flotan hacia arriba, así que
          // necesitamos padding-top extra en el grid para que no se
          // topen con el heading anterior. También forzamos que todas
          // las cards se estiren al alto de la más grande (`items-stretch`
          // por default del grid + `h-full` en la card asegura consistencia).
...
function OrbCompactCard({
  ...
  return (
    <a
      ...
      style={{
        // Base: teal (harbor) translúcido en vez de navy oscuro. Se
        // siente como cristal ahumado azul-claro en vez de negro. El
        // gradient va de teal-medium → teal-alto, dejando pasar más
        // luminosidad del gradient sky detrás.
        background:
    ...
    >
      {/* Grid tech cyan tenue — ahora cubre TODA la card sin máscara
          radial. El grid es la textura de fondo del instrumento, no un
          spot para dirigir al ojo. Sigue con `tech-grid-pulse` que ya
          da un latido lento al opacity. */}
      <div
        aria-hidden="true"
      ...
      {/* Nodos tech-blink — tres puntitos cyan que parpadean en
          posiciones espaciadas por la card, dando la sensación de
          "sensores activos" a lo ancho del componente. */}
      <span
        aria-hidden="true"
      ...
      {/* Zona texto — derecha, tinta clara sobre teal translúcido.
          `pl-5 sm:pl-7` separa del borde derecho de la lente. Text-shadow
          sutil da lift extra al título contra el gradient tras la card. */}
...
function OrbHaloStackedCard({ product: p, seeLabel }: CardProps) {
  return (
    ...
    >
      {/* Producto flotante — sin container ni background, sin halo
          irradiado. El "glow glacier" ahora vive PEGADO a la silueta
          del producto vía múltiples drop-shadow cyan (uno corto + uno
          medio) → se lee como un contorno luminoso que rodea el
          producto, no como un halo separado que ocupa espacio propio.
          Padding interno reducido (p-1 en vez de p-4) para que
          productos con silueta horizontal (Sensores, PigVision) usen
          más de la zona disponible y se vean del mismo peso visual
          que los verticales (Insylo). */}
      <div className="pointer-events-none absolute inset-x-0 top-0 z-10 mx-auto h-48 w-48 sm:h-56 sm:w-56">
        <Image
          ...
          style={{
            filter:
              // 1er drop-shadow: borde cyan brillante muy cercano
              //   (blur 4px) → hace de "borde encendido" del producto.
              // 2do drop-shadow: halo cyan medio (blur 10px) → aire
              //   glacier que envuelve la silueta sin alejarse mucho.
              // 3er drop-shadow: sombra vertical navy sutil para grounding.
      ...
      {/* Card glacier glass — sólo texto. Padding-top generoso para
          dejar espacio bajo el producto flotante que se le encima.
          `flex-1` la hace crecer para ocupar todo el alto disponible en
          la celda del grid → todas las cards del row igualan altura,
          sin importar cuánto texto tenga cada tagline. */}
      <div
        ...
        style={{
          // Glass glacier equilibrado: blanco dominante (80% arriba →
          // 70% abajo) con tinte cyan MUY sutil sólo al final. Antes
          // el gradient iba sky→blanco→cyan que competía con el
          // navy del texto; ahora el fondo cede al contenido.
          background:
      ...
      >
        ...
        {/* mt-auto pega el CTA al fondo de la card — así todas las
            cards muestran el "Conocer …" en la misma línea vertical,
            aunque el tagline sea más corto en unas que en otras. */}
