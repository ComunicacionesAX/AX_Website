...
export function FeatureAccordionCard({
  ...
  // Cuando el consumidor NO pasa `progress`, la barra se comporta como
  // antes: scaleX 0→1 al abrir (decorativa). Cuando SÍ lo pasa, la
  // barra se convierte en un progress fill controlado (0..1 = tiempo
  // del autoplay).
  ...
  return (
    <Tag
      ...
      style={
        open
          ? {
              // Glass navy translúcido + shimmer cyan interior. Se
              // siente como instrumento oscuro esmerilado en vez de
              // un bloque navy plano.
              background:
    ...
    >
      ...
      {/* Contenedor animado por altura (grid trick).
          `grid-rows-[0fr]` → `grid-rows-[1fr]` anima la altura de forma
          smooth sin conocer el alto real del contenido.
          `origin-top` asegura que la expansión/colapso ocurre DESDE
          arriba hacia abajo (abrir) y de abajo hacia arriba (cerrar).
          Duración un poco más larga (400ms) para que la dirección se
          perciba mejor. */}
      <div
      ...
      >
        <div className="min-h-0 origin-top">
          {/* Barra glaciar de progreso. En modo controlado (progress
              prop), pinta un track sutil + fill que crece con el tiempo
              del autoplay. En modo decorativo (sin prop), sólo el fill
              se ve, comportamiento original scaleX 0→1 al abrir. */}
          <div
            aria-hidden="true"
          ...
          {/* Body: slide vertical desde arriba (-6px) cuando abre;
              se retira hacia arriba (-4px) al cerrar. delay-100 al
              abrir crea la sensación de "el texto llega DESPUÉS de que
              la card ya se abrió". Al cerrar sin delay, el texto se
              va antes. */}
          <p
