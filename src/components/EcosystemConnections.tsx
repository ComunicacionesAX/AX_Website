...
export function EcosystemConnections({ productName, rows, columns }: Props) {
  // Íconos por fila — coinciden con el orden fijo:
  // 0: Función principal → target
  // 1: Cómo usa los datos → workflow
  // 2: Tipo de dato utilizado → database
  const rowIcons = [Target, Workflow, Database];
  const single = columns.length === 1;

  return (
    // Cuando hay una sola card, la centramos y limitamos su ancho.
    // Con múltiples cards, usamos grid de 2 columnas para que se lean
    // en paralelo.
    <div
      className={
        single
